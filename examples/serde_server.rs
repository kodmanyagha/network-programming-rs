...
fn main() {
    ...
    if args[1] == "--server" {
        ...
        let listener = TcpListener::bind("0.0.0.0:9990").expect("Could not bind");
        ...
        let mut stream = TcpStream::connect("127.0.0.1:9990").expect("Could not connect to server");

        loop {
            print!("Please provide a 3D point as three comma separated integers: ");
            ...
            let input = str::from_utf8(&buffer).expect("Could not writebuffer as string");
            if input.is_empty() {
